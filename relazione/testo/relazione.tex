...
\includepdf[angle=90,pagecommand=\section{Schema relazionale finale}\thispagestyle{empty}]{../resources/pdf/schemaLogico.pdf}
...
SELECT 50 * (1 - percentuale / 100) AS prezzo_scontato FROM SCONTI WHERE fascia = (
	SELECT fascia FROM SCONTI WHERE eta_soglia <= 25 ORDER BY eta_soglia DESC LIMIT 1
);
...
INSERT INTO ORDINI (prezzo, cliente, pagamento) VALUES (
	(SELECT prezzo_scontato FROM SCONTI WHERE fascia = (
		SELECT fascia FROM SCONTI WHERE eta_soglia <= 25 ORDER BY eta_soglia DESC LIMIT 1
	)), -- prezzo scontato del biglietto
	'cliente@email.com', -- email del cliente
...
UPDATE BIGLIETTI SET ordine = (SELECT MAX(id) FROM ORDINI) WHERE id = (SELECT MAX(id) FROM BIGLIETTI);
...
SELECT id_evento, COUNT(*) AS biglietti_venduti FROM BIGLIETTI WHERE ordine IS NOT NULL AND data <= '2023-12-31' GROUP BY id_evento;
